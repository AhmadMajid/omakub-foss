...
# GNOME Extensions installation - Debian compatible version
...
if [ "$ID" = "ubuntu" ]; then
  ...
  echo "Disabled Ubuntu-specific extensions"
else
  echo "Skipping Ubuntu extension disabling on $ID"
fi

# Pause to assure user is ready to accept confirmations
gum confirm "To install Gnome extensions, you need to accept some confirmations. Ready?"

# Install new extensions (should work on both Ubuntu and Debian)
...
echo "Configuring extension schemas..."
...
if [ "$ID" = "debian" ]; then
  echo "Note: Extensions should work on Debian, but you may need to logout/login or restart GNOME Shell (Alt+F2, type 'r', press Enter)"
fi
